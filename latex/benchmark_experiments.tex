...
\title{\vspace{-1.5em}\textbf{TimeTensors: Forecasting Benchmark and Experiments}\vspace{-0.4em}}
...
The launcher compares exactly seven policies: keep all pairs;
\code{remove_train_windows}, \code{remove_eval_windows}, or
\code{remove_all_windows}; and \code{drop_train_users},
\code{drop_eval_users}, or \code{drop_all_users}. Train and evaluation
...
population. Curated source exclusions from each dataset's \code{config.json}
...
Full and ultra use
\[
  (L,H)\in\{(168,24),(336,48),(504,168),(720,168),(720,720)\}
\]
...
history, metadata, loss, and completion artifacts; a signature-matched
\code{run.complete} permits exact resumption, and tables are regenerated only
when the selected grid is complete and newer than their workflow marker.
