...
document defines the forecasting task, the eight implemented experiment
...
\code{RevIN} class, with affine parameters disabled and enabled,
...
and their row-aligned user, query, and run identities. Per-user and worst-tail
summaries are derived from those rows rather than stored as duplicated loss
tensors.
...
\]
With $N=\sum_u n_u$, the two population dispersions are
\[
  S_{\mathrm{elem}}
  =\sqrt{\frac1N\sum_u\sum_i(\ell_{u,i}-M_{\mathrm{elem}})^2},\qquad
  S_{\mathrm{user}}
  =\sqrt{\frac1U\sum_u(\overline\ell_u-M_{\mathrm{user}})^2}.
\]
...
\]
Central and per-user artifacts use the same metric names and reductions.
For each loss name $m$, \code{m} is $M_{\mathrm{elem}}$,
\code{std_m} is $S_{\mathrm{elem}}$, \code{user_m} is
$M_{\mathrm{user}}$, \code{std_user_m} is $S_{\mathrm{user}}$, and
\code{w10_m} is $\Wten$. Thus \code{mse}, \code{user_mse}, and
\code{w10_mse} compare central and per-user models under identical weighting
rules; the same five summaries are recorded for nMSE, MAE, nMAE, and relative
MSE whenever those losses are evaluated.

\section{Experiment 8: frozen foundation models}
\label{sec:foundation}

The eighth family evaluates four frozen adapters without optimizer updates:
Chronos-2, Chronos-Bolt, Chronos-T5, and TS-ICL. Every adapter receives
the same TimeTensors query rows and writes the aligned five-metric loss
contract. Model-native preprocessing and released checkpoint inference remain
inside each official package; the evaluator only constrains accessible data,
dates, and optional covariates.

The root \code{08_foundation_models.slurm} defaults to
\code{STAGES=evaluate,tables}. Test uses Electricity at $504{:}168$, seed 1.
Full uses the common six-dataset, three-setting grid and seeds 1--3; ultra
selects the same frozen model axis. Reports compare element-weighted MSE,
equal-user MSE, and worst-10\%-user MSE.

\section{Experimental details}
\label{sec:details}

Every numbered DGX front has a matching \code{_selena.slurm} front that sources
the same family implementation. \code{LOGS_ROOT} and \code{OUTPUTS_ROOT}
default to \path{logs/} and \path{outputs/}; Selena overrides them to
\path{logs_selena/} and \path{outputs_selena/}. Family \code{OUT_ROOT} values
derive from the selected output root, so cluster placement changes storage but
not experiment identity or scientific configuration.
...
\code{timetensors} object overrides them, and explicit run values override
both. For \code{drop_users}, null inherits, an empty list retains every CSV
user, and a nonempty run list replaces the configured default. Exclusions are
applied only while reading CSV. After aggregation, \code{missing_values}
defaults to \code{zero}, replacing NaNs with zero; \code{error} rejects
them, and infinite values are always rejected. Prepared tensors are also
validated as finite when they are loaded.

Prepared tensors live under the project's
\path{datasets/prepared/dataset/signature/}. The signature contains only the
effective tensor-building configuration; every complete variant has a version-1
\path{dataset_manifest.json} with source provenance, effective preparation
settings, shapes, and artifact inventory. Tensor loading never reapplies user
exclusions.

Filtered TIME panels are prepared by \code{python -m scripts.prepare_time_csv}.
The shared implementation selects an immutable
\code{Real-TSF/TIME-ProcessedCSV} revision, merges only files with identical
timestamp indices, and writes one portable wide CSV/config pair plus
\code{datasets/time/catalog.json}. Prepared panels can be selected with a
slash-qualified dataset name and use the same dataset-config precedence.
...
\centering
\small
\caption{Models exercised by the eight publication launchers.}
\label{tab:models}
...
\code{chronos2} & Thin official Chronos-2 adapter; TimeTensorModel may add configured covariates without changing this alias. & Frozen \\
\code{chronos_bolt} & Thin official Chronos-Bolt adapter; non-empty covariates are rejected. & Frozen \\
\code{chronos_t5} & Official Chronos-T5 sampling pipeline with the median sample forecast. & Frozen \\
\code{ts_icl} & Official TS-ICL checkpoint and native preprocessing through a tensor adapter. & Frozen \\
\bottomrule
\end{tabularx}
\end{table}

These four foundation names are the sole lower-snake-case aliases shared with
TSFM evaluation and online adaptation. Project wrappers compose around the
same basic adapters and are configured independently of model identity.
...
\noindent PatchTST and DLinear are pinned source-adapted packages:
\begin{flushleft}\small
PatchTST upstream: \texttt{yuqinie98/PatchTST}\\
Revision: \texttt{204c21efe0b39603ad6e2ca640ef5896646ab1a9}\\[2pt]
DLinear upstream: \texttt{cure-lab/LTSF-Linear}\\
Revision: \texttt{0c113668a3b88c4c4ee586b8c5ec3e539c4de5a6}
\end{flushleft}
The four frozen evaluation
adapters and TIME preparation implementation are byte-identical to TSFM
...
evaluated without optimization. Every model in the foundation family likewise
sets $E=0$ and skips training.

Evaluation performs one inference pass per configured run. On each selected
split, \code{all_losses.pt} stores row-aligned elementwise MSE, nMSE, MAE,
nMAE, and relative-MSE tensors; compact \code{individual_ids},
\code{query_ids}, and \code{run_ids}; an ID-to-name map; and all five scalar
element/user/tail summaries defined above. The optional example forecast is
captured during that same pass. No \code{per_user_all_losses.pt} copy is written. Tables use \code{test1},
average each selected metric within a seed, then report the mean and sample
standard deviation across seeds. Each row uses an explicit $\times10^m$ scale.
Every seed directory contains model, history, metadata, and the sole aligned
loss artifact.

\subsection{Result identities and manifests}
Every family continues below the selected output root as
\path{family/dataset/L_H/backbone/configs/run_n/seed_n}. The ordered
...
linear models; \code{scope} for central/per-user; and \code{normalization} for
foundation models. No synthetic upper/lower
...
Run signatures use only the schema, identity/model and pipeline/experiment
configs, and seeds. Files are not fingerprinted for reuse; provenance is ignored
and code/data reruns are manual. Only a global contract break changes the schema;
\code{new} creates an unchanged-parameter repeat.

The current contract uses the thesis-wide \code{schema_version: 1}; this number
versions the shared manifest structure rather than the project-specific loss
payload. The former duplicated per-user outputs were deliberately deleted, so
only newly computed runs using the sole aligned \code{all_losses.pt} contract
can exist locally. The four overall states are
\code{not_run}, \code{running}, \code{interrupted}, and \code{completed}; seed
state may additionally be \code{ready}. The default
...
table failures preserve completed runs and interrupt only unfinished work. Completed manifests are authoritative, without
later file hashing or synchronization checks. Tables support
distinct/latest/average config handling and selected/latest/distinct/average
repeat selection; explicit pipeline filters select configurations and must
match even with one run. \code{SELECTED_RUNS.txt} stores exact-repeat selection
and each report stores requested filters and obtained input manifests. A future
downstream run may bind its signature to an upstream run's schema, identity,
pipeline/experiment configuration, and exact seeds while retaining paths and
manifest IDs as provenance only; single-run resolution fails closed across
pipeline variants. No pre-restart output or log remains locally, and every
desired experiment configuration must be recomputed.
...
The dependency-light synthetic smoke explicitly enumerates every expected
method in the seven trainable/control families and requires seeds 1 and 2 for
each. The separate foundation checks use fake or constructor-only adapters and
never load publication checkpoints. The synthetic run tree is below
\path{outputs/synthetic_smoke/family/synthetic_smoke/24_6/}
\path{numpy_linear_proxy/method/run_n/seed_n}; reports use
\path{outputs/reports/synthetic_smoke}.
