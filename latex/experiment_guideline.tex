...
The eighth family evaluates five frozen adapters without optimizer updates:
Chronos-2, Chronos-Bolt, TS-ICL, and TabPFN-TS. TiRex-2 remains implemented but
is excluded from the launcher until explicitly restored. Every active adapter receives
...
\code{tirex2} & Official TiRex-2 checkpoint, native preprocessing, and test-time augmentation through a tensor adapter. & Frozen \\
\code{tabpfn_ts} & In-context TabPFN-TS inference through the official TabPFN package and checkpoint. & Frozen \\
\bottomrule
\end{tabularx}
\end{table}

These five foundation names are the sole lower-snake-case aliases shared with
...
\code{family/dataset/L_H/backbone/configs/run_n/seed_n}. The ordered
