...
def train_sklearn_stage(
    ...
    *,
    ...
    history_path = save_torch({"fit": fit_info, "train": [], "train_batch": [], "valid": {}}, out_dir / "train_history.pt")
...
def main(config: Mapping[str, Any] | None = None) -> dict[str, Any]:
    plain = to_plain_config(config or {})
    setup_logging(section(plain, "misc").get("log_level", "INFO"))
    configs = seeded_configs(plain)
    if len(configs) == 1:
        return train_sklearn_stage(configs[0])
    return {int(seed(item)): train_sklearn_stage(item) for item in configs}
